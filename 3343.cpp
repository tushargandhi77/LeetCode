...
public:
    ...
    int solve(int digit,int evenIndexdigitcount,int currsum,vector<int>& freq,vector<long long>& fermatFact,vector<vector<vector<int>>>& t){
            if(digit == 10){
            ...
            if(t[digit][evenIndexdigitcount][currsum] != -1) return t[digit][evenIndexdigitcount][currsum];
            ...
            for(int count = 0;count <= min(freq[digit],(n+1)/2 -evenIndexdigitcount );count++){
                ...
                long long val = solve(digit+1,evenIndexdigitcount+evenPosCount,currsum+(digit*count),freq,fermatFact,t);

                ways = (ways + (val*div)%M)%M;
            }
            return t[digit][evenIndexdigitcount][currsum] = ways;
    }
